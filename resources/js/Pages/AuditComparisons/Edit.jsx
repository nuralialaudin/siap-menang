...
export default function Edit({ auth, audit, voters }) {
    const { data, setData, put, processing, errors } = useForm({
        voter_id: audit.voter_id || '',
        volunteer_sentiment: audit.volunteer_sentiment || 'ABU-ABU',
        survey_sentiment: audit.survey_sentiment || 'ABU-ABU',
        is_match: audit.is_match ? 1 : 0,
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('audit-comparisons.update', audit.id), {
            onError: () => {
                showToast('error', 'Gagal memperbarui data audit.');
            }
        });
    };

    return (
        <AuthenticatedLayout user={auth.user} header={<h2 className="font-semibold text-xl text-gray-800">Edit Audit Perbandingan Sentimen</h2>}>
